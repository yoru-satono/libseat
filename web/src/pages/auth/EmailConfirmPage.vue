...
onMounted(async () => {
  ...
  if (!token) { state.value = 'error'; message.value = '缺少确认令牌'; return }
  try {
    ...
    state.value = 'error'
    message.value = e?.message || '确认失败，链接可能已过期'
  }
})
</script>
  ...
  <AuthLayout>
    <div class="text-center py-6">
      ...
      <div v-if="state === 'loading'" class="space-y-3">
        ...
        <p class="text-surface-500 text-sm">正在确认邮箱…</p>
      </div>
      ...
      <div v-else-if="state === 'success'" class="animate-scale-in">
        ...
        <p class="font-semibold text-surface-900 text-lg font-display">邮箱变更已确认！</p>
        <p class="text-sm text-surface-500 mt-1">即将跳转到个人资料…</p>
      </div>
      ...
      <div v-else class="animate-fade-in">
        ...
        <p class="font-semibold text-surface-900 text-lg font-display">确认失败</p>
        <p class="text-sm text-surface-500 mt-1">{{ message }}</p>
        <RouterLink to="/profile" class="mt-5 inline-block">
          <span class="px-5 py-2 text-sm font-medium bg-primary-600 text-white rounded-button hover:bg-primary-700 transition-colors">返回个人资料</span>
        </RouterLink>
      </div>
    </div>
  </AuthLayout>
